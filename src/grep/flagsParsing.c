...
static void ProcessFlagsAndMap(int argc, char*** argv, Flags* flags, Map* map, Regex* patterns) {
	int mnum = 0;
...
void ConcatPattern(char*** argv, Regex* patterns) {
	for (int i = 0, j = 0; i < patterns->pnum; i += 3) {
		if (patterns->efidx[i] == 'e') {
			...
			filep = fopen((*argv)[patterns->efidx[i + 2]] + patterns->efidx[i+1], "r");
...
static void ProcessPatterns(char*** argv, Flags* flags, Map* map, Regex* patterns) { 
	if (patterns->pnum)
...
void ParseFlags(int argc, char*** argv, Flags* flags, Map* map, Regex* patterns) {
	ProcessFlagsAndMap(argc, argv, flags, map, patterns);
	ProcessPatterns(argv, flags, map, patterns);
